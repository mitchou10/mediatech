...
if __name__ == "__main__":
    ...
    if config.get("type") == "dila_folder":
        ...
        downloader = DilaDownloader(
            config, f"data/unprocessed/{download_name}", pattern=regular_pattern
        )
        ...
        if download_name == "travail_emploi":
            from src.download.sheets import TravailDownloader

            downloader = TravailDownloader(config, f"data/unprocessed/{download_name}")
            patterns = []
        else:
