...
with DAG(
    ...
    wait_for_cnil = ExternalTaskSensor(
        task_id="wait_for_cnil",
        external_dag_id="CNIL",
    ...
    download_and_process_files = BashOperator(
        ...
        bash_command="mediatech download_and_process_files --source constit --model BAAI/bge-m3",
    )

    export_table = BashOperator(
        task_id="export_table", bash_command="mediatech export_table --table constit"
    )

    upload_dataset = BashOperator(
        task_id="upload_dataset",
        bash_command="mediatech upload_dataset --dataset-name constit",
    )
