...
with DAG(
    ...
    wait_for_constit = ExternalTaskSensor(
        task_id="wait_for_constit",
        external_dag_id="CONSTIT",
    ...
    download_and_process_files = BashOperator(
        ...
        bash_command="mediatech download_and_process_files --source dole --model BAAI/bge-m3",
    )

    export_table = BashOperator(
        task_id="export_table", bash_command="mediatech export_table --table dole"
    )

    upload_dataset = BashOperator(
        task_id="upload_dataset",
        bash_command="mediatech upload_dataset --dataset-name dole",
    )
