...
class AdministrationsDirectoryExporter(BaseExporter):
    def process(
        self,
        user_id: str,
        dataset_name: str = "administrations_directory",
        *args: tuple,
        **kwargs: dict,
    ) -> None:
        ...
        commit_message = (
            f"Data update on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        dataset.push_to_hub(
            repo_id=repo_id,
            split="train",
...
class StateAdministrationsDirectoryExporter(AdministrationsDirectoryExporter):
    def process(
        self,
        user_id: str,
        dataset_name: str = "state_administrations_directory",
        *args: tuple,
        **kwargs: dict,
    ) -> None:
...
class LocalAdministrationsDirectoryExporter(AdministrationsDirectoryExporter):
    def process(
        self,
        user_id: str,
        dataset_name: str = "local_administrations_directory",
        *args: tuple,
        **kwargs: dict,
    ) -> None:
        super().process(user_id=user_id, dataset_name=dataset_name, *args, **kwargs)
